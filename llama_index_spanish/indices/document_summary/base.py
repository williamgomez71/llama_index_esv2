...
from llama_index_spanish.indices.utils import embed_nodes
...
from llama_index_spanish.schema import (
    BaseNode,
    NodeRelationship,
    NodeWithScore,
    RelatedNodeInfo,
...
from llama_index_spanish.storage.docstore.types import RefDocInfo
...
DEFAULT_SUMMARY_QUERY = (
    ...
    "Describe también algunas de las preguntas que este texto puede responder."
)
...
class DocumentSummaryIndex(BaseIndex[IndexDocumentSummary]):
    ...
    def get_summary_by_page(
        ...
        """Retrieve nodes."""
        summary_ids = self._index_struct.summary_ids
        summaries = []
        for idx in summary_ids:
            summary_text = self.docstore.get_node(idx).get_content()
            summary_id = self._index_struct.summary_id_to_node_ids[idx]
            summary_nodes_id = self.docstore.get_nodes(summary_id)
            if page != None:
