...
from llama_index_spanish.data_structs.data_structs import IndexDict, IndexStructType
from llama_index_spanish.indices.managed.base import BaseManagedIndex, IndexType
from llama_index_spanish.schema import BaseNode, Document, MetadataMode, TextNode
...
class VectaraIndex(BaseManagedIndex):
    ...
    def as_query_engine(self, **kwargs: Any) -> BaseQueryEngine:
        if kwargs.get("summary_enabled", True):
            from llama_index_spanish.indices.managed.vectara.query import VectaraQueryEngine

            kwargs["summary_enabled"] = True
            retriever = self.as_retriever(**kwargs)
            return VectaraQueryEngine.from_args(retriever, **kwargs)  # type: ignore
        else:
            from llama_index_spanish.query_engine.retriever_query_engine import (
