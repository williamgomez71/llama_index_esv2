...
def get_system_and_user_prompt(history: List[ChatMessage]):
    ...
    for chat in history:
        ...
        if chat.role == "user":
            user_prompt= chat.content
    print(system_prompt, user_prompt)
    return system_prompt, user_prompt

def format_prompt(instruction, system_prompt=None, conversation="", languaje="English"):
    # Prompt
...
def find_more_similar_tool(objetivo, lista = None):
    ...
    if "Action:" in objetivo:
        ...
        return objetivo
    chat_message= ChatMessage(content=mensaje_final)
