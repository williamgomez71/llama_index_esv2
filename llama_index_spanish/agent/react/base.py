...
from llama_index_spanish.agent.react.output_parser import ReActOutputParser
from llama_index_spanish.agent.react.types import (
    ActionReasoningStep,
    BaseReasoningStep,
    ObservationReasoningStep,
    ResponseReasoningStep,
)
from llama_index_spanish.agent.types import BaseAgent
from llama_index_spanish.callbacks import (
    CallbackManager,
    CBEventType,
    EventPayload,
    trace_method,
)
from llama_index_spanish.chat_engine.types import AgentChatResponse, StreamingAgentChatResponse
from llama_index_spanish.llms.base import LLM, ChatMessage, ChatResponse, MessageRole
from llama_index_spanish.llms.openai import OpenAI
from llama_index_spanish.memory.chat_memory_buffer import ChatMemoryBuffer
from llama_index_spanish.memory.types import BaseMemory
...
class ReActAgent(BaseAgent):
    ...
    def _extract_reasoning_step(
        ...
        try:
            ...
            print_text(reasoning_step, "red")
            print_text(reasoning_step.is_done, "pink")
    ...
    def _process_actions(
        ...
        print_text(f"{reasoning_step}\n", color="red")
        print_text(f"{tool}\n", color="red")
        print_text(f"{reasoning_step.action_input}\n", color="pink")
    ...
    def chat(
        ...
        for _ in range(self._max_iterations):
            ...
            # send prompt
            system_prompt, user_prompt =  get_system_and_user_prompt(input_chat)
            input_chat = format_prompt(system_prompt + user_prompt )
            chat_response = self._llm.complete(input_chat)
            chat_response = find_more_similar_tool(chat_response.text, tools)
            print_text(chat_response, "yellow")
            # given react prompt outputs, call tools or return response
            reasoning_steps, is_done = self._process_actions(
                tools, output=chat_response
            )
            current_reasoning.extend(reasoning_steps)
            response , is_done = self.post_process_answer(reasoning_steps, message)
            #current_reasoning.extend(response)
            if is_done:
                return response
                break
    ...
    def post_process_answer(self, reasoning_steps, message):
        message = message
        print_text(message, "green")
        print_text(reasoning_steps, "red")
        try:
