...
def set_global_tokenizer(tokenizer: Union[Tokenizer, Callable[[str], list]]) -> None:
    ...
    if isinstance(tokenizer, Tokenizer):
        llama_index_spanish.global_tokenizer = tokenizer.encode
    else:
        llama_index_spanish.global_tokenizer = tokenizer


def get_tokenizer() -> Callable[[str], List]:
    import llama_index_spanish

    if llama_index_spanish.global_tokenizer is None:
        tiktoken_import_err = (
            "`tiktoken` package not found, please run `pip install tiktoken`"
        )
        try:
    ...
    assert llama_index_spanish.global_tokenizer is not None
    return llama_index_spanish.global_tokenizer
